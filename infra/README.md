...
reverses the process. Both expect to be run on the EC2 box, from inside
...
**2. Give the EC2 instance permission to write to it**, via an IAM
...
**3. Install the AWS CLI on the EC2 box** (needed for the upload step):
...
```
0 3 * * * /home/ubuntu/event-platform-api/infra/backup-db.sh >> /home/ubuntu/db-backups/backup.log 2>&1
```

(Adjust the path if the repo isn't cloned to `/home/ubuntu/event-platform-api`.)

## Restoring

```bash
./infra/restore-db.sh /home/ubuntu/db-backups/event_platform_20260101_030000.sql.gz
```
...
```bash
aws s3 cp s3://<bucket>/event_platform_20260101_030000.sql.gz /tmp/restore.sql.gz
...
None of this applies to `docker-compose.yml` (local dev) — that database is
disposable by design. These scripts only target `docker-compose.prod.yml`.
