...
# to S3 for offsite durability (protects against losing the EC2 instance
# or its disk entirely, not just accidental data changes).
#
# Run from anywhere; it cd's into the project root itself. Meant to run on
# the EC2 box via cron, e.g. daily at 03:00:
...
FILENAME="event_platform_${TIMESTAMP}.sql.gz"
...
find "$BACKUP_DIR" -name 'event_platform_*.sql.gz' -mtime "+${RETENTION_DAYS}" -delete
