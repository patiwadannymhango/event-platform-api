...
No SMS provider credentials have been configured yet, so this defaults to
a "console" backend that just logs the message via the Notification
model (status=SENT, provider_response={"backend": "console"}) without
actually dispatching anything — this keeps the rest of the notification
pipeline (registration confirmations, payment confirmations) fully
working end-to-end today, and the switch to a real provider is a single
settings change.
